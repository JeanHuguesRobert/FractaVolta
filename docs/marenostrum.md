...
lang: en
description: "MareNostrum is the strategic framework for sovereign Mediterranean compute infrastructure: energy, compute, cognition, and civic continuity. FractaVolta is its operational reference layer."
canonical_url: https://fractavolta.com/marenostrum
last_stamped_at: 2026-06-02
date: "2026-06-02"
status: "active — English strategic framework entry point"
---
...
- **Mediterranean-anchored.** The stranded-solar thesis depends on the Mediterranean basin's specific geography: high irradiation, saturated cable interconnections, and multi-state coastlines.
- **Public-private-academic.** The framework resists capture by any single actor. A consortium that includes only states, only corporations, or only universities will not work.
- **Non-financial substrate.** The CXU is priced in exergy and service level, not in fiat. Financial layers can attach later; the substrate must be technically auditable first.
- **Open-source toolchain.** No proprietary lock-in at any layer. MareNostrum specifies; FractaVolta operates; Cogentia tools the methodology; Inseme provides the application platform.

FractaVolta's role in the consortium would be operational: the reference implementation. We design, build, and run the first sites. The framework explains why each design choice goes the way it does.

## Relationship to FractaVolta

MareNostrum specifies; FractaVolta builds.

The Mariani Village deployments, the DC-native node architecture, and the Inference Packet Network demonstrators are operational expressions of the MareNostrum framework on Corsica.

A different operator implementing the same framework in Sicily, Sardinia, the Balearic Islands, the Greek Aegean, or the Maghreb coast would be consistent with the proposal. The framework is the public good. Operations are how it becomes real on a given territory.
...
Early-stage. The corpus is published and stable. The consortium is unformed. The operational reference implementation exists at pilot scale through FractaVolta. The Mediterranean-wide network is the long horizon.
